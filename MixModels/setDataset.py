...
def give_rows(df_list, key_column, common_rows):
    # 遍历所有DataFrame，删除差异行
    updated_dfs = []
    for df in df_list:
...
def spilt_dataset(union_dfs, dataset_folder, column_for_diff, exclude_columns):
    ...
    negative_diff = diff_df[column_for_diff] < 0.02
    pos = 0
    for df in union_dfs:
        # 列交换
...
data_folder = './Training_set/APP_0/'
dataset_folder = './Testing_set/'
seqlen = 4

label_files = [file_name for file_name in os.listdir(data_folder)]
df_list = []
# label_files[-seqlen:]，文件内容仅为xlsx时
for label_file in label_files[-seqlen:]:
